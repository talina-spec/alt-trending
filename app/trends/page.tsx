...
export default function TrendsScreen() {
  ...
  // лайки храним в localStorage
  const [likes, setLikes] = useState<Record<string, boolean>>({});
  useEffect(() => {
    try {
      const raw = window.localStorage.getItem("yt_likes");
      if (raw) setLikes(JSON.parse(raw));
    } catch {}
  }, []);
  useEffect(() => {
    try {
      window.localStorage.setItem("yt_likes", JSON.stringify(likes));
    } catch {}
  }, [likes]);
  ...
  // Подгружаем сразу при первом заходе
  useEffect(() => { if (videos.length === 0) fetchBatch(true); /* eslint-disable-next-line */ }, []);
